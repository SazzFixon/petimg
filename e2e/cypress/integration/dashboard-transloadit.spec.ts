describe('Dashboard with Transloadit', () => {
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ petimg }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(petimg.getPlugin('Transloadit').activeAssemblies).every(
        ...
        cy.wait(['@delete']).then(() => {
          ...
          // @ts-ignore fix me
          expect(
            Object.values(petimg.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ petimg }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      petimg.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ petimg }) => {
      ...
      const { files } = petimg.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      petimg.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ petimg }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(petimg.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ petimg }) => {
      const { files } = petimg.getState()
      petimg.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        petimgserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'petimg-core:3.2.0,petimg-transloadit:3.1.3,petimg-tus:3.1.0,petimg-dropbox:3.1.1,petimg-box:2.1.1,petimg-facebook:3.1.1,petimg-google-drive:3.1.1,petimg-instagram:3.1.1,petimg-onedrive:3.1.1,petimg-zoom:2.1.1,petimg-url:3.3.1',
    ...
    cy.get('.petimg-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.petimg-StatusBar-actions > .petimg-c-btn').click()
    cy.wait([
