...
const packageNames = await fs.readdir(new URL('../packages/@petimg', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@petimg/core is automatically included',
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@petimg/${pgk}'`).join('\n')}
...
const prettyPath = (url) => url.toString().split('petimg', 2)[1]
