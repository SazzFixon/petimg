...
async function compileCSS () {
  ...
  const files = await glob('packages/{,@petimg/}*/src/style.scss')
  ...
  for (const file of files) {
    ...
    if (outputDir.includes(path.normalize('packages/petimg/'))) {
      outfile = path.join(outputDir, 'petimg.css')
    }
    await mkdir(outputDir, { recursive: true })
    await writeFile(outfile, postcssResult.css)
    console.info(
      chalk.green('✓ Built petimg CSS:'),
