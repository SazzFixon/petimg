# petimg + AWS S3 Example

This example uses @petimg/companion with a custom AWS S3 configuration.
...
`@petimg/companion` in a `.env` file. You may find useful to first copy the
`.env.example` file:
...
corepack yarn workspace @petimg-example/aws-companion start
```
