...
import getFileTypeIcon from '@petimg/dashboard/lib/utils/getFileTypeIcon.js'
...
function petimgDashboardFileIcon ({ type }) {
...
export default function FileList ({ petimg }) {
  const petimgFiles = petimg.store.state.files
  const petimgFilesArray = Object.keys(petimgFiles).map((id) => petimgFiles[id])

  return (
    <View style={styles.container}>
      <FlatList
        data={petimgFilesArray}
          ...
          return (
            <View style={styles.item}>
              {item.type === 'image' ? (
                ...
                <petimgDashboardFileIcon type={item.type} />
              )}
