...
class MarkdownTextarea {
  ...
  setuppetimg = () => {
    this.petimg = new petimg({ autoProceed: true })
      .use(Transloadit, {
    ...
    this.petimg.on('complete', (result) => {
  ...
  uploadFiles = (files) => {
    const filesForpetimg = files.map(file => {
      return {
        data: file,
        type: file.type,
        name: file.name,
    ...
    this.petimg.addFiles(filesForpetimg)
  }
}
